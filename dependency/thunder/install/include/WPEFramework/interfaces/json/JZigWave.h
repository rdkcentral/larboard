...
namespace Exchange {
    ...
    namespace JZigWave {
        ...
        static void Register(JSONRPC& _module_, IZigWave* _impl_)
        {
            ...
            // Indexed Property: 'permutable' - To allow new devices to the network, the controller should be placed
            _module_.Register<JsonData::ZigWave::PermutableData, Core::JSON::Boolean, std::function<uint32_t(const string&,
                     const JsonData::ZigWave::PermutableData&, Core::JSON::Boolean&)>>(_T("permutable"), 
                [_impl_](const string& _index_, const JsonData::ZigWave::PermutableData& params, Core::JSON::Boolean& result) -> uint32_t {
                    uint32_t _errorCode;

                    uint32_t _indexConverted_{};

                    if (Core::FromString(_index_, _indexConverted_) == false) {
            ...
            // Indexed Property: 'accept' - To allow new devices to the network, the controller should be placed
            _module_.Register<void, Core::JSON::Boolean, std::function<uint32_t(const string&, Core::JSON::Boolean&)>>(_T("accept"), 
                [_impl_](const string& _index_, Core::JSON::Boolean& result) -> uint32_t {
                    uint32_t _errorCode;
                    ...
                    if (Core::FromString(_index_, _indexConverted_) == false) {
                        ...
                        if (params.IsSet() == false) {
                            ...
                            _errorCode = _impl_->Permutable(_indexConverted_);
            ...
            // Indexed Property: 'device' - To allow new devices to the network, the controller should be placed (r/o)
