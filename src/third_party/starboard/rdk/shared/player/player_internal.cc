...
using starboard::CreateDecryptorElement;
using starboard::CodecToGstCaps;
...
class ScopedUnlock {
 public:
  explicit ScopedUnlock(std::mutex& mutex)
    : mutex_(mutex) {
    mutex_.unlock();
  }
  ~ScopedUnlock() {
...
#undef CASE
  ...
  void DecoderNeedsData(std::lock_guard<std::mutex>&, MediaType media, bool can_delay = true) {
  ...
  void HandleInititialSeek(std::lock_guard<std::mutex>&);
  void DidEnd();
...
void PlayerImpl::HandleInititialSeek(std::lock_guard<std::mutex>& lock) {
...
void ForceStop() {
  using starboard::GetPlayerRegistry;
