...
void debug_log_override(
 GstDebugCategory *category, GstDebugLevel level,
 const gchar *file, const gchar *function,
 gint line, GObject *object, GstDebugMessage *message, gpointer data) {
  gchar *log_line = gst_debug_log_get_line(category, level, file, function, line, object, message);
  gint64 ts = g_get_monotonic_time();
  fprintf(stderr, "%.010lu.%.06lu %s", ts / G_USEC_PER_SEC, ts % G_USEC_PER_SEC, log_line);
  g_free(log_line);
}
...
int SbRunStarboardMain(int argc, char** argv, SbEventHandleCallback callback) {
  ...
  if (const char *env = std::getenv("COBALT_OVERRIDE_GST_DEBUG_LOG"); env && g_str_equal(env, "1")) {
