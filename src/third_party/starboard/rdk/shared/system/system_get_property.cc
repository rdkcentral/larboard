...
bool GetLimitAdTracking(char* out_value, int value_length) {
  ...
  if (auto opt_out = platform::advertising().limit_ad_tracking(); opt_out.has_value()) {
    SB_LOG(INFO) << "Pravakar GetLimitAdTracking: value from platform::advertising(): " << (*opt_out ? "1" : "0");
...
bool GetAdvertisingId(char* out_value, int value_length) {
  ...
  if (auto ad_id = platform::advertising().advertising_id(); ad_id.has_value()) {
    SB_LOG(INFO) << "Pravakar GetAdvertisingId: value from platform::advertising(): " << *ad_id;
